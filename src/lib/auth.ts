...
export function getAuthContext(provider: AuthProvider): AuthContext {
  if (provider === 'clerk') {
    const hasKey = Boolean(getPublicEnv('VITE_CLERK_PUBLISHABLE_KEY'))
    return {
      provider,
      isConfigured: hasKey,
      ...
      hint: hasKey
        ...
        : 'Set VITE_CLERK_PUBLISHABLE_KEY (build or /runtime-config.json) to enable Clerk auth.',
    }
  }

  const hasUrl = Boolean(getPublicEnv('VITE_SUPABASE_URL'))
  const hasAnon = Boolean(getPublicEnv('VITE_SUPABASE_ANON_KEY'))
  ...
  return {
    ...
    hint: isConfigured
      ...
      : 'Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY (build env or /runtime-config.json) to enable Supabase auth.',
  }
}
